module ParallelPSO2 (optimize) where
...
initializeSwarm swarmSize dimension bounds objectiveFunction = do
    ...
    let swarm = map (\pos -> createParticle pos dimension objectiveFunction) positions `using` parListChunk 64 rdeepseq
...
optimize swarmSize dimension objectiveFunction bounds c1 c2 w iterations = do
    ...
    where
        ...
        iterateSwarm swarm globalBestScore globalBestPosition n = do
            ...
            let updatedSwarm = map (\(p, (r1, r2)) -> updateParticle p globalBestPosition objectiveFunction bounds c1 c2 w r1 r2) (zip swarm randomPairs) `using` parListChunk 64 rdeepseq
